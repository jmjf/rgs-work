...
export const GameMath = {
   ...
   getArrayRange: (min: number, max: number) => Array.from({ length: max - min + 1 }, (_, i) => min + i),
   ...
   randomSumIn: (arr: number[], max: number) => {
     ...
     for (let i = 0; i < arr.length; i++) {
       for (let j = 0, len = sets.length; j < len; j++) {
         ...
         const candidateSum = GameMath.sum(candidateSet);
         if (candidateSum <= max) {
     ...
     return sums[GameMath.random(0, sums.length - 1)];
   },
 };