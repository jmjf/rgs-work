...
Changing logic to detect won (available = none) vs. lost (time = 0) to display a conditional message (replace `isGameDone` with `gameStatus` which is win, lose, or playing).
   * Philosophy: avoid overlapping variables (like `isWon` and `isLost`) in favor of an explicit status -> clearer
...
Now, need to reset the timer on game reset. And to do that, we need to clean any side effects. Better way to do this is unmount the component, which will trigger clean side effects.
